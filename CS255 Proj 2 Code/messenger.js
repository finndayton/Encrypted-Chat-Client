...
export default class MessengerClient {
  ...
  async sendMessage(name, plaintext) {    
    ...
    if (!(name in this.conns)) {
      ...
      //deal with skipped messages

      //Ns: # of mks in sending chain
      //Nr: # of mks in receiving chain
      //PN: # of mks in previous sending chain
      //MKSKIPPED: dict (pk_sender, Nr) -> mk. Stores all the missed mk_r keys.

      this.conns[name].Ns = 0; 
      this.conns[name].Nr = 0; //there is no receiving chain yet
      this.conns[name].PN = 0;
      this.conns[name].MKSKIPPED = {};
    }
    ...
    //form header 
    // N: the message’s number in the sending chain (N =0,1,2,. . . ) AND 
    ...
    const header = {vGov: new_gov_pair.pub, 
      ...
      N: this.conns[name].Ns,
      PN: this.conns[name].PN
     }; 
  ...
  async receiveMessage(name, [header, ciphertext]) {
    ...
    // how to detect a message arrived out of order here?
    // if header.Ns (number of mks in the sender's sending chain) is more than 1 larger than our Nr, we have a problem. 
    // we need to symetric ratchet forward our receive chain, storing the intermediate keys in this.conns[name].MKSKIPPED.
    // we should also store the last key
    while (header.N > this.conns[name].Nr) {
      const ck_r = await HMACtoHMACKey(this.conns[name].ck_r, "ck-str");
      this.conns[name].ck_r = ck_r;
      const mk = await HMACtoAESKey(this.conns[name].ck_r, "mk-str");
      this.conns[name].Nr++;
      this.conns[name].MKSKIPPED[this.conns[name].Nr] = mk;
    ...
    const mk = this.conns[name].MKSKIPPED[this.conns[name].Nr]
